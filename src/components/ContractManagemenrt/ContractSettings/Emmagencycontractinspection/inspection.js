...
class Inspection extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      if (!state.contractid && state.projectid) {
        ...
        const { data: business } =
          await ContractData.getcontractBycontractID(state.contractid);
        if (!business) {
  ...
  getPagedData = () => {
    ...
    else if (selectedrole && selectedrole.contractid)
      filtered = allsources.filter(
        (m) => m.Business.contractid === selectedrole.contractid
      );
    ///////////////////////////////////////////contractdiscription projecttypename  contractmode
      ...
      const brochure = business.map((business, index) => {
        return (
          <tr key={business.contractid}>
            ...
            <td>{business.projecttypename}</td>
            <td>{business.contractmode}</td>
