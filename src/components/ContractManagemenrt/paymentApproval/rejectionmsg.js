...
class Rejectionmsg extends Component {
  ...
  async handleSave(e) {
    try {
      ...
      const Rejectionlevel = this.state.paymentstatus;
      const itemrejectedon = "Contract";
      const statuse = "rected on " + this.state.paymentstatus;
     
      e.preventDefault();
     
      if (this.state.paymentstatus === "New") {
        const statuse = "rejected on second level";
        
        await PaymentData.contractpaymentupdatestatus(
          this.state.contractpaymentid,
          statuse
        );
      } else if (this.state.paymentstatus === "rejected on first level") {
        const statuse = "rejected on second level";
        await PaymentData.contractpaymentupdatestatus(
          this.state.contractpaymentid,
          statuse
        );
      } else if (this.state.paymentstatus === "Verified") {
      ...
      if (ex.response && ex.response.status === 400) {
        ...
        toast.error(
          "An Error Occured, while Updating inspection status data Please try again later" +
            ex
        );
      }
    }
                ...
                <div className="text-muted text-center mt-2 mb-3">
                  <h1>
                    <div style={{ textAlign: "center" }}>
                      <h1>RMF Payment- Reject Payment</h1>
                    </div>
                  </h1>
                </div>
